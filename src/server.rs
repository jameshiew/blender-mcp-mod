...
impl ServerHandler for BlenderServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().enable_prompts().build())
            ...
            .with_instructions("Control Blender through its MCP extension. Start with get_addon_status and get_scene_info; follow next_offset when more objects are needed. Inspect exact object names before editing. Use get_object_info(details=true) for concise material, modifier, and animation summaries; use get_material_info, get_node_group_info, get_modifier_info, and get_animation_info for focused inspection. Follow page offsets and check truncation/omitted fields before assuming inspection is complete. Use set_camera for lens, projection, and camera framing; use set_viewport for view direction, framing, zoom, and shading. Use execute_blender_code for modeling, materials, and scene operations, and Sketchfab when external assets suit the task. Batch related edits, print concise results, and verify visible changes with get_viewport_screenshot. For camera renders, use start_render, poll get_render_status, and retrieve get_render_image; use cancel_render to stop a job. For recoverable edits, opt into checkpoint and summarize_changes on execute_blender_code. Errors and timeouts can leave partial changes: use get_execution_result for opted-in calls and inspect before retrying. restore_checkpoint saves a safety checkpoint, opens a working copy, and clears all namespaces. Credit assets using returned attribution. JSON results are available as structuredContent and text; image tools also return capture metadata.")
    }
